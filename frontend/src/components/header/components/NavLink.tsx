    ...
    >
      {subLinks.length > 0 ? (
        <button className="flex items-center h-full font-semibold">
          <span
            className={`h-full flex flex-col justify-center font-semibold after:block after:h-[2px] after:justify-self-start after:w-full after:bg-secondary after:transition-all ${
              isActive
                ? "after:max-w-[60%]"
                : hovered
                ? "after:max-w-[80%]"
          ...
          <img className="max-h-[0.8em] ml-2" src={linkArrow} alt="" />
        </button>
      ) : (
        <Link
          to={to}
          className={`${
            className && className
          } font-semibold h-full self-stretch min-h-full relative`}
        >
          <span
            className={`h-full flex flex-col justify-center font-semibold after:block after:h-[2px] after:justify-self-start after:w-full after:bg-secondary after:transition-all ${
              isActive
...
const SubNavLink = ({ title, to, subLinks }: NavLinkProps) => {
  ...
  return (
    <div
      className="relative px-6 py-3 bg-white z-50"
      onMouseEnter={() => setHovered(true)}
      onMouseLeave={() => setHovered(false)}
    >
      {!to ? (
        <button className="flex items-center h-full font-semibold">
          ...
          <img
            className="max-h-[0.8em] ml-2 -rotate-90"
            src={linkArrow}
            alt=""
          />
        </button>
      ) : (
        <Link className="font-semibold flex items-center" to={to}>
          <span
            className={`h-full flex flex-col justify-center font-semibold after:block after:h-[2px] after:justify-self-start after:w-full after:bg-secondary after:transition-all ${
              isActive
                ? "after:max-w-[60%]"
